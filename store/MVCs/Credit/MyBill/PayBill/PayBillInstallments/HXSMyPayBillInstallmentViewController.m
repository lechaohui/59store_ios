...
@property (nonatomic, assign) BOOL                                  isConfirmTheInstallment;  // 已经确定分期
...
- (void)dealloc
{
    self.myNewPayBillInstallmentFooterView.delegate = nil;
}
...
{
    self.currentBillListEntity = entity;
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    if (_selectEntityArray
        && !_isConfirmTheInstallment) {
        rows = 4;
    } else if (_isConfirmTheInstallment
              && _currentConfirmEntity) {
        rows = 4;
    } else {
        // Do nothing
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(!_isConfirmTheInstallment
       && (2 == indexPath.row)) { //点击分期期数
        
        if(!_selectEntityArray) {
            return;
        }
        
        NSMutableArray *totalMonthArray = [[NSMutableArray alloc] initWithCapacity:5];
