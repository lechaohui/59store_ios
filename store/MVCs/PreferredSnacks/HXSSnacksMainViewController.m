...
#import "HXSSnacksMainViewController.h"
#import "HXSShop.h"
...
@interface HXSSnacksMainViewController () <HXSDormListHorizontalViewControllerDelegate,
                                         HXSFloatingCartViewDelegate,UISearchBarDelegate>
...
@implementation HXSSnacksMainViewController
...
+ (instancetype)createDromVCWithShopId:(NSNumber*)shopIdIntNum
{
    ...
    HXSSnacksMainViewController* mainVC = [mainStoryBoard instantiateViewControllerWithIdentifier:@"HXSSnacksMainViewController"];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [self changeNavigationBarBackgroundColor:[UIColor colorWithHexString:@"fde25c"]
                        pushBackButItemImage:nil
                     presentBackButItemImage:nil
...
- (void)initialSearchBar{
    ...
    //搜索框
    UISearchBar *searchBar=[[UISearchBar alloc]initWithFrame:CGRectMake(40, 0, SCREEN_WIDTH-80, 44)];
    ...
    //购物车图标
    UIButton *cartBut = [Maker makeBtn:CGRectMake(SCREEN_WIDTH-30, 5, 30, 30) title:nil img:@"gouwuche" font:nil target:self action:@selector(cartAction:)];
    [self.navigationController.navigationBar addSubview:cartBut];
...
- (void)initialButtonStatus
{
    ...
    [self.checkoutBtn setTitle:[NSString stringWithFormat:@"%d元起送", self.shopEntity.minAmountFloatNum.intValue]
                      forState:UIControlStateNormal];
...
- (void)cartUpdated:(NSNotification*)noti
{
    ...
    if ((MAX(promotionInfoModel.itemAmountDecNum.doubleValue, promotionInfoModel.originAmountDecNum.doubleValue) >= self.shopEntity.minAmountFloatNum.floatValue)
        ...
        if (MAX(promotionInfoModel.itemAmountDecNum.doubleValue, promotionInfoModel.originAmountDecNum.doubleValue) <= 0.00) {
            [self.checkoutBtn setTitle:[NSString stringWithFormat:@"%d元起送", self.shopEntity.minAmountFloatNum.intValue] forState:UIControlStateDisabled];
        } else {
