...
@interface HXSBannerHeaderView () <UIScrollViewDelegate>
...
@implementation HXSBannerHeaderView
...
- (void)layoutSubviews
{
    ...
    if(self.slideItems.count > 0) {
        HXSSlideItem * item = [self.slideItems objectAtIndex:0];
...
- (void)setSlideItemsArray:(NSArray *)slideItemsArray
{
    ...
    [self.slideItems addObjectsFromArray:slideItemsArray];
    
    for(int i=0; i<self.slideItems.count; i++) {
        HXSSlideItem * item = [self.slideItems objectAtIndex:i];
        UIImageView * image = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
        [image sd_setImageWithURL:[NSURL URLWithString:item.image]  placeholderImage:[UIImage imageNamed:@"img_kp_banner_cat"]];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if(contentOffsetX <= 0) {
        self.currentPageIndex = [self getValidNextPageIndexWithPageIndex:self.currentPageIndex - 1];
        //        DLog(@"previous，当前页:%d",self.currentPageIndex);
        [self configContentViews];
    }
}
...
- (void)tapGestureAction:(UITapGestureRecognizer *)tap {
    ...
    if(CGRectContainsPoint(self.scrollView.frame, point)) {
        if(self.slideItems.count > self.pageControl.currentPage) {
            HXSSlideItem * slideItem = [self.slideItems objectAtIndex:self.pageControl.currentPage];
            if(self.eventDelegate && [self.eventDelegate respondsToSelector:@selector(onStartEvent:)]) {
                [self.eventDelegate onStartEvent:slideItem.clickEvent];
            }
        }
    }
}
