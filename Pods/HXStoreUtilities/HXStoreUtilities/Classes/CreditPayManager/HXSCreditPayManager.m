...
#define HXS_USAGE_EVENT_BOX_APPLY_CREDIT_PAY @"box_apply_baihuahua"
...
{
    /** 0 @"消费类账单"  1  @"分期类账单" */
    NSDictionary *dic = @{@"bill_type": @"0"};
    UIViewController *myBillVC = [[HXSMediator sharedInstance] HXSMediator_billViewControllerWithParams:dic];
    
    UINavigationController *nav = [[HXSMediator sharedInstance] HXSMediator_rootViewControllerNavigation];
...
- (void)jumpToGetPayPasswordVerifyViewController:(CreditPayResultCallBack)completion
{
    ...
    UIViewController *passwdVc = [[HXSMediator sharedInstance] HXSMediator_forgotPasswordViewController];
