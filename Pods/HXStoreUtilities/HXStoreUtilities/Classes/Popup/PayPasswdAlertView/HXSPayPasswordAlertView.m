...
static CGFloat MESSAGE_LABEL_DEFAULT_HEIGHT = 20.0f;
static CGFloat MESSAGE_LABEL_EDGE_PADDING   = 25.0f;
...
- (HXSPayPasswdView *)createPayPasswordView
{
    ...
    NSString *bundlePath = [bundle pathForResource:@"Popup" ofType:@"bundle"];
    if (bundlePath) {
        bundle = [NSBundle bundleWithPath:bundlePath];
    }
    
    NSArray *viewsArr = [bundle loadNibNamed:@"HXSPayPasswdView"
                                                      owner:nil
                                                    options:nil];
...
- (void)setupFrameOfAlertView:(HXSPayPasswdView *)alertView
{
    ...
    if ((nil == self.messageStr)
        ...
        frame.size.height -= MESSAGE_LABEL_DEFAULT_HEIGHT;
        ...
        CGFloat messagelabelWidth = width - MESSAGE_LABEL_EDGE_PADDING * 2;
        ...
        CGRect frame = alertView.frame;
        frame.size.width = width;
        if (labelSize.height > MESSAGE_LABEL_DEFAULT_HEIGHT) {
            frame.size.height += labelSize.height - MESSAGE_LABEL_DEFAULT_HEIGHT;
        }
