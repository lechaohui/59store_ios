...
- (void)onClickShareBtn:(UIButton *)btn
{
    ...
    if (32 * 1000 < [imageData length]) { // size is bigger than 32K
        scaleImage = [image imageByScalingToMaxSize];
    } else {
        scaleImage = image;
    }
    ...
    switch (shareType) {
        case kHXSShareTypeWechatFriends:
        {
            ...
            [[HXSWXApiManager sharedManager] shareToWeixinWithTitle:title text:text image:scaleImage url:url timeLine:NO callback:self.shareCallBack];
        }
            break;
        ...
        case kHXSShareTypeWechatMoments:
        {
            ...
            [[HXSWXApiManager sharedManager] shareToWeixinWithTitle:text text:text image:scaleImage url:url timeLine:YES callback:self.shareCallBack];
        }
            break;
        ...
        case kHXSShareTypeQQMoments:
        {
            if(![[HXSQQSdkManager sharedManager] isQQInstalled]) {
                [MBProgressHUD showInViewWithoutIndicator:self status:@"您当前没有安装QQ" afterDelay:1.5];
                return;
            }
