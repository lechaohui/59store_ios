...

{
    ...
    HXWebServiceModel *model = [HXStoreWebService createWebServiceModelWithPath:path];
                             ...
                             failure:(void (^)(HXSErrorCode status, NSString *msg, NSDictionary * data))failure
{
    ...
    if ([HXStoreWebService containedUrl:path]) {
        ...
        [[HXStoreWebService webService] updateRequestSerializerHeadFieldWithDic:headerFieldMDic model:model];
...
+ (HXWebServiceModel *)createWebServiceModelWithPath:(NSString *)pathStr
{
    ...
    if (HXSEnvironmentProduct == [[ApplicationSettings instance] currentEnvironmentType]) {
        // ip address
        ipAddressStr = [HXStoreWebService fetchIPByURLStr:baseURLStr];
    }
