...
/**继续上传 */
@property (weak ,nonatomic) IBOutlet UIButton                       *continueToUploadButton;
...
{
    HXSMyPhotosPrintCartViewController *vc = [HXSMyPhotosPrintCartViewController controllerFromXibWithModuleName:@"HXStorePrint"];
...
- (void)initTableView
{
    ...
    [_mainTableView registerNib:[UINib nibWithNibName:NSStringFromClass([HXSPrintCartTableViewCell class]) bundle:bundle] forCellReuseIdentifier:NSStringFromClass([HXSPrintCartTableViewCell class])];
}
...
- (void)onClickTopView:(UITapGestureRecognizer *)tap
{
    if (self.hasDisplayShopInfoView) {
        ...
        [self.view insertSubview:_shopInfoVC.view belowSubview:_topView];
...
- (void)fetchShopEntityAndCheckIsClosedNetworking
{
    __weak typeof(self) weakSelf = self;
    ...
    [HXSPrintModel fetchShopInfoWithSiteId:manager.currentSite.site_id
                                  ...
                                  complete:^(HXSErrorCode status, NSString *message, HXSShopEntity *shopEntity)
     {
         ...
         [weakSelf jumpToCheckOutViewController];
     }];
}
...
{
    HXSPrintCheckoutViewController *checkoutViewController = [HXSPrintCheckoutViewController controllerFromXibWithModuleName:@"HXStorePrint"];
    [checkoutViewController initPrintCheckoutViewControllerWithEntity:_printCartEntity
                                                    andWithShopEntity:_shop];
    __weak typeof(self) weakSelf = self;
...
{
    HXSPrintSettingViewController *printSettingVC = [HXSPrintSettingViewController controllerFromXibWithModuleName:@"HXStorePrint"];
    ...
    [self addSettingView:printSettingVC];
}

- (void)addSettingView:(HXSPrintSettingViewController *)printSettingVC
{
    if (SYSTEM_VERSION_GREATER_THAN(@"8.0")) {
        ...
        [self.navigationController addChildViewController:printSettingVC];
        [self.navigationController.view addSubview:printSettingVC.view];
        [printSettingVC.view mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.navigationController.view.mas_bottom);
            make.height.equalTo(self.navigationController.view.mas_height);
            make.left.equalTo(self.navigationController.view);
            make.right.equalTo(self.navigationController.view);
        }];
        [printSettingVC didMoveToParentViewController:self.navigationController];
...
{
    [self fetchShopEntityAndCheckIsClosedNetworking];
}
...
- (IBAction)confirmCartButtonAction:(id)sender
{
    ...
    if(needToUploadItemNums == 0) { // 无需上传
        [self fetchShopEntityAndCheckIsClosedNetworking];
        return;
    } else {
...
- (void)backBarButtonAction
{
    ...
    HXSCustomAlertView *alertView = [[HXSCustomAlertView alloc] initWithTitle:@"确定退出购物车?"
                                                                      message:@"注意:该操作会清空购物车"
                                                              leftButtonTitle:@"取消"
                                                            rightButtonTitles:@"退出"];
    __weak typeof(self) weakSelf = self;
...
{
    if(!_backBarButton) {
        UIImage *backImage = [HXSPrintModel imageFromNewName:@"btn_back_normal"];
