...
-(void)initialTableView
{
    ...
    [self.tableView registerNib:[UINib nibWithNibName:HXSPrintManagerDeliveryInfoCellIdentify bundle:bundle] forCellReuseIdentifier:HXSPrintManagerDeliveryInfoCellIdentify];
    [self.tableView registerNib:[UINib nibWithNibName:HXSPrintSelfDeliveryInfoCellIdentify bundle:bundle] forCellReuseIdentifier:HXSPrintSelfDeliveryInfoCellIdentify];
}
...
-(void)initialDeliveryTimeStrArr
{
    ...
    if(self.selectDeliveryEntity
       && self.selectDeliveryEntity.deliveryTimesMutArr
       && self.selectDeliveryEntity.deliveryTimesMutArr.count > 0){
        ...
        if(kHXSNoError == status) {
            ...
            if(deliveries&&deliveries.count > 0) {
                ...
                if(HXSPrintDeliveryTypeShopOwner == weakSelf.selectDeliveryEntity.sendTypeIntNum.intValue) {
                    if(weakSelf.selectDeliveryEntity.deliveryTimesMutArr
                       && weakSelf.selectDeliveryEntity.deliveryTimesMutArr.count > 0) {
...
{
    if(section < self.deliveriesMutArr.count
       && self.selectDeliveryEntity) {
...
{
    if(indexPath.section < self.deliveriesMutArr.count
       && self.selectDeliveryEntity) {
...
{
    if(indexPath.section < self.self.deliveriesMutArr.count
       && self.selectDeliveryEntity) {
        HXSDeliveryEntity *temp = self.deliveriesMutArr[indexPath.section];
        if(HXSPrintDeliveryTypeShopOwner == temp.sendTypeIntNum.intValue) {
            ...
            if(0 == indexPath.row) {
                ...
                [cell.textLabel mas_remakeConstraints:^(MASConstraintMaker *make) {
                    make.left.equalTo(cell).offset(45);
...
- (void)selectButtonClicked:(NSIndexPath *)indexPath
{
    ...
    if(HXSPrintDeliveryTypeShopOwner == self.selectDeliveryEntity.sendTypeIntNum.intValue) {
        if(self.selectDeliveryEntity.deliveryTimesMutArr
           && self.selectDeliveryEntity.deliveryTimesMutArr.count > 0) {
