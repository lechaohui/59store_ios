...
@interface HXSAddressDecorationView () <HXSCitySelectionDelegate,
                                        HXSSiteSelectionDelegate,
                                        HXSBuildAreaSelectionDelegate,
                                        HXSBuildingSelectionDelegate>
{
    UIViewController *_currentController;
}

@property (nonatomic, weak) id<HXSAddressDecorationViewDelegate> selectionDelegate;
...
- (instancetype)initWithDelegate:(id<HXSAddressDecorationViewDelegate>) delegate
...
- (HXSBuildingAreaSelectVC *)buildingAreaCtrl
{
    if (_buildingAreaCtrl == nil) {
        ...
        _buildingAreaCtrl.delegate = self;
...
{
    if ([self.selectionDelegate respondsToSelector:@selector(addressSelectCity:)]) {
        [self.selectionDelegate addressSelectCity:city];
    }
}
...
- (void)refreshCityInfo:(HXSCity *)city
{
    ...
    if ([self.selectionDelegate respondsToSelector:@selector(refreshCityInfo:)]) {
        [self.selectionDelegate refreshCityInfo:city];
    }
}
...
{
    if ([self.selectionDelegate respondsToSelector:@selector(addressSelectSite:)]) {
        [self.selectionDelegate addressSelectSite:site];
    }
}
...
- (void)refreshSiteInfo:(HXSSite*)site
{
    ...
    if ([self.selectionDelegate respondsToSelector:@selector(refreshSiteInfo:)]) {
        [self.selectionDelegate refreshSiteInfo:site];
    }
}

- (void)addressSelectBuildArea:(HXSBuildingArea *)buildingArea
{
    if ([self.selectionDelegate respondsToSelector:@selector(addressSelectBuildArea:)]) {
        [self.selectionDelegate addressSelectBuildArea:buildingArea];
    }
}
...
- (void)refreshBuildingAreaInfo:(HXSBuildingArea *)buildingArea
{
    ...
    if ([self.selectionDelegate respondsToSelector:@selector(refreshBuildingAreaInfo:)]) {
        [self.selectionDelegate refreshBuildingAreaInfo:buildingArea];
    }
}

- (void)addressSelectBuilding:(HXSBuildingEntry *)building
{
    if ([self.selectionDelegate respondsToSelector:@selector(addressSelectBuilding:)]) {
        [self.selectionDelegate addressSelectBuilding:building];
    }
}

- (void)refreshBuildingBuildingInfo:(HXSBuildingEntry *)building
{
    if ([self.selectionDelegate respondsToSelector:@selector(refreshBuildingBuildingInfo:)]) {
        [self.selectionDelegate refreshBuildingBuildingInfo:building];
    }
}
