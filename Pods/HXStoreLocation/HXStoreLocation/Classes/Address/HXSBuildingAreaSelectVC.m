...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(self.selectedBuildingArea&&[self.selectedBuildingArea.name isEqualToString:temp.name]){
        ...
        // 把当前最近的值保存
        if (_selectedBuildingArea != temp) {
            _selectedBuildingArea = temp;
            if ([self.delegate respondsToSelector:@selector(refreshBuildingAreaInfo:)]) {
                [self.delegate refreshBuildingAreaInfo:temp];
            }
        }
