...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BOOL hasopen  = (0 < [nameEntity.shopsArr count]) ? YES : NO;
    
    if (hasopen) {
        
        cell.accessoryView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"ye"]];
        
    }else{
        
        UILabel *tipLab = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 50, 20)];
        tipLab.text = @"暂未开通";
        [tipLab sizeToFit];
    ...
    if(self.selectedBuilding &&[self.selectedBuilding.buildingNameStr isEqualToString:nameEntity.buildingNameStr]&&[self.selectedBuilding.dormentryIDNum isEqualToNumber:nameEntity.dormentryIDIntNum]) {
        ...
        buildingEntry.buildingNameStr = nameEntity.buildingNameStr;
        buildingEntry.dormentryIDNum  = nameEntity.dormentryIDIntNum;
        buildingEntry.hasOpened       = (0 < [nameEntity.shopsArr count]) ? YES : NO;
        
        _selectedBuilding = buildingEntry;
        
        if([self.delegate respondsToSelector:@selector(refreshBuildingBuildingInfo)]){
            [self.delegate refreshBuildingBuildingInfo:buildingEntry];
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    DLog(@"店铺个数：%ld",entity.shopsArr.count);
