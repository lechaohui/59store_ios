...
- (NSString *)currentPrintURL {
    return @"http://fecdn.59store.com/activity/bzydios?t=1476414355431";
}

- (NSString *)currentDocLibURL {
    return @"http://fecdn.59store.com/activity/fxzqhelp?t=1476414507959";
}
...
/*
 * 招行银行
 * http://192.168.30.111:8080/pay/cmb?order_id=%@&money=%0.2f&token=%@
 * 传入 订单号， 金额， token
 */
- (NSString *)cmbPayURL
{
    ...
    switch(type) {
        case HXSEnvironmentProduct:
            return @"http://pay.59store.com/pay/cmb?order_id=%@&money=%0.2f&token=%@";
        case HXSEnvironmentTemai:
            return @"http://pay-dev.59store.net/pay/cmb?order_id=%@&money=%0.2f&token=%@";
        case HXSEnvironmentStage:
            return @"http://pay.59store.net/pay/cmb?order_id=%@&money=%0.2f&token=%@";
        case HXSEnvironmentQA:
            return @"http://pay.59shangcheng.com/pay/cmb?order_id=%@&money=%0.2f&token=%@";
        default:
            break;
    }
    return @"http://pay.59store.com/pay/cmb?order_id=%@&money=%0.2f&token=%@";
...
- (NSArray *)hosts
{
    ...
    NSURL *serviceHttpsURLProduct = [NSURL URLWithString:HXSServiceHttpsURLProduct];
    NSURL *serviceHttpsURLTest    = [NSURL URLWithString:HXSServiceHttpsURLTest];
    NSURL *serviceHttpsURLStage   = [NSURL URLWithString:HXSServiceHttpsURLStage];
    NSURL *serviceHttpsURLQA      = [NSURL URLWithString:HXSServiceHttpsURLQA];
    
    // H5 web address
    NSURL *serviceFecdnURL  = [NSURL URLWithString:@"fecdn.59store.com"];   // js、css、部分图片、新的活动页
    NSURL *serviceGalaURL   = [NSURL URLWithString:@"gala.59store.com"];    // 老的活动页
    NSURL *serviceYTURL     = [NSURL URLWithString:@"yt.59store.com"];      // 约团C端
    NSURL *serviceMKURL     = [NSURL URLWithString:@"mk.59store.com"];      // 约团B端
    NSURL *serviceDormURL   = [NSURL URLWithString:@"dorm.59store.com"];    // 店长后台
    NSURL *serviceImageURL  = [NSURL URLWithString:@"image.59store.com"];   // 以前又拍云上的一些图片吧，现在很少用了
    NSURL *serviceXYHHRURL  = [NSURL URLWithString:@"xyhhr.59store.com"];   // 校园合伙人
    ...
    return @[serviceURLProductUrl.host,
             ...
             serviceURLQA.host,
             serviceHttpsURLProduct.host,
             serviceHttpsURLTest.host,
             serviceHttpsURLStage.host,
             serviceHttpsURLQA.host,
             serviceFecdnURL.host,
             serviceGalaURL.host,
             serviceYTURL.host,
             serviceMKURL.host,
             serviceDormURL.host,
             serviceImageURL.host,
             serviceXYHHRURL.host,
             ];
}
