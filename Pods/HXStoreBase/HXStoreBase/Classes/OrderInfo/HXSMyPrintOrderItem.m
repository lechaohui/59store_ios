...
{
    NSDictionary *itemMapping = @{@"pdfPathStr":             @"pdf_path",
                                  @"pdfMd5Str":              @"pdf_md5",
...
- (void)encodeWithCoder:(NSCoder *)aCoder
{
    ...
    [aCoder encodeObject:self.isFromLibraryDocumentNum forKey:@"isFromLibraryDocumentNum"];
    [aCoder encodeObject:self.docIdStr forKey:@"docIdStr"];
    [aCoder encodeObject:[[NSNumber alloc]initWithInteger:self.archiveDocTypeNum] forKey:@"archiveDocTypeNum"];
}
...
- (nullable instancetype)initWithCoder:(NSCoder *)aDecoder;
{
    ...
    if (self)
    {
        ...
        _isFromLibraryDocumentNum   = [aDecoder decodeObjectForKey:@"isFromLibraryDocumentNum"];
        _docIdStr                   = [aDecoder decodeObjectForKey:@"docIdStr"];
        _archiveDocTypeNum          = [[aDecoder decodeObjectForKey:@"archiveDocTypeNum"] integerValue];
    }
    return self;
}
...
- (id)copyWithZone:(NSZone *)zone
{
    ...
    [copy setIsFromLibraryDocumentNum:self.isFromLibraryDocumentNum];
    [copy setDocIdStr:self.docIdStr];
