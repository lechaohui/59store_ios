...
+ (JSONKeyMapper *)keyMapper
{
    NSDictionary *mapping = [[NSDictionary alloc] initWithObjectsAndKeys:
                             ...
                             @"discount_desc",         @"discountDescStr", nil];
...
- (id)initWithOrderInfo:(id)orderinfo
{
    ...
    if(self) {
        ...
        } else if ([orderinfo isKindOfClass:[HXSOrderCreateModel class]]) {
            HXSOrderCreateModel *info = (HXSOrderCreateModel *)orderinfo;
            
            self.food_amount = @(0);
            self.order_amount = [NSNumber numberWithDouble:([info.payAmountStr doubleValue] / 100.0)]; // 分 转化为 元
            self.order_sn = info.orderIDStr;
            self.attach = nil;
            self.type = [info.orderTypeStr intValue];
            self.add_time = @([info.createTimeStr longLongValue]);
            self.discount = @([info.createTimeStr doubleValue]);
            self.activityInfos = nil;
            
            /** 自定义字段 */
            HXSOrderCreateItemModel *item = info.itemsArr[0];
            NSString *titleStr            = item.nameStr;
            self.orderDescriptionStr      = titleStr;
        } else if ([orderinfo isKindOfClass:[HXSPrintDocumentOrderInfo class]]) {
            HXSPrintDocumentOrderInfo *info = (HXSPrintDocumentOrderInfo *)orderinfo;
            
            self.order_amount = info.amountDecNum;
            self.order_sn     = info.idStr;
            self.paytype      = info.payTypeNum.intValue;
            self.add_time     = info.createTimeNum;
            self.type         = info.typeIdNum.intValue;
